...
class Backtester:
    ...
    def fetch_historical_data(self):
        ...
        while current_ts < end_ts:
            try:
                ...
                self.logger.debug(f"Fetched {len(klines)} candles from {datetime.fromtimestamp(klines[0][0]/1000)}")
        ...
        for i in range(1, len(data)):
            ...
            if self.position:
                ...
                if self.position["side"] == "buy":
                    equity += self.position["size"] * (current["close"] - self.position["entry_price"])
                else:  # sell
                    equity += self.position["size"] * (self.position["entry_price"] - current["close"])
    ...
    def _close_position(self, price, timestamp):
        ...
        trade = {
            ...
            "profit_loss_percent": (profit_loss / (self.position["entry_price"] * self.position["size"])) * 100,
        }
